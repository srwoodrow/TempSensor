...
// To check functionality of each chip
...
void setup()
{
  ...
  // Pins for debugging
  pinMode(getTemp, OUTPUT);
  pinMode(getRH, OUTPUT);
...
void measure()
{
  digitalWrite(getTemp, HIGH);
  // Get temperature data from the ADT7310
  fetch_temp();
  delay(50);
  digitalWrite(getTemp, LOW);
  
  digitalWrite(toSerial, HIGH);
  Serial.print("T = ");
  Serial.println(temperature);
  delay(50);
  digitalWrite(toSerial, LOW);
  
  // Get humidity data from HIH6130 - error byte should be zero if no errors
  digitalWrite(getRH, HIGH);
  byte hum_error = fetch_humidity();
  delay(50);
  digitalWrite(getRH, LOW);
